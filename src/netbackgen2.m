...
% General neural net w/ differentiable transfer trained using backpropagation.
    properties
        ...
        % We keep a whole backward network.
        backnet
    end
    ...
    % ==================================================
    methods
        function self = netbackgen2(varargin)
            % Usage: netbp(arch, data) OR netbp(fullarch, TransferFcns, ErrorFcns)
            [fullarch, TransferFcns, ErrorFcns] = netbackgen2.processArgs(varargin{:});
            % Instantiate
            ...
            % Instantiate backnet. Use netdt because it has methods we
            % need.
            self.backnet = netdt(fullarch(end:-1:1), TransferFcns, ErrorFcns);
            self.ErrorFcn = ErrorFcns{end};
            assert(strcmp(ErrorFcns{end}, 'CrossEntropyError'));
            self.ErrorFcns = ErrorFcns;
            for i = 1:length(ErrorFcns)
                % Assume all crossent for now.
                assert(strcmp(ErrorFcns{i}, 'CrossEntropyError'));
            end
            % Initialize weights
            self = initialize(self);
            self.backnet = initialize(self.backnet);
        end
        function self = train(self, data)
            ...
            for i = 1:self.nepoch
                for j = 1:nbatch
                    ...
                    % Backward net
                    %Y = getStochasticActivations(self.backnet, batchdata{j}.target);
                    Y = getActivations(self.backnet, batchdata{j}.target);
                    % Update
                    for k = 1:self.nlayers
                       ...
                       % Update backward net.
                       backdata.input  = layerdata.target;
                       backdata.target = layerdata.input;
                       self.backnet.layers{self.nlayers-k+1} = ...
                           train_gd_nobias(self.backnet.layers{self.nlayers-k+1}, ...
                                    backdata, self.nupdate);
                    end
                end % batch
                % Record errors
                self = recordErrors(self, data);
